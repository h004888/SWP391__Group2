...
var e = {
    ...
    dashboardChart: function () {
        var ac = e.select('#ChartPayoutOfYear');
        if (e.isVariableDefined(ac)) {
            ...
            // Get revenue data from the controller
            var revenueData = window.revenuePerMonth || {};
            ...
            while (currentDate <= endDate) {
                var monthStr = currentDate.toISOString().slice(0, 7); // Format: YYYY-MM
                allMonths.push(monthStr);
                currentDate.setMonth(currentDate.getMonth() + 1);
            }

            // Prepare data arrays for the chart
            var revenueValues = allMonths.map(function(month) {
                return revenueData[month] || 0;
            });

            // Format dates for display
            var formattedDates = allMonths.map(function(month) {
            ...
            window.revenueChart = new ApexCharts(ac, options);
            ...
            var yearlyRevenueEl = document.getElementById('yearlyRevenue');
            
            if (monthlyRevenueEl) {
                monthlyRevenueEl.textContent = new Intl.NumberFormat('vi-VN', {
                    style: 'currency',
                    currency: 'VND'
                }).format(totalRevenue);
            }
            if (yearlyRevenueEl) {
                yearlyRevenueEl.textContent = new Intl.NumberFormat('vi-VN', {
                    style: 'currency',
                    currency: 'VND'
                }).format(totalRevenue);
            }
    ...
    earningChart: function () {
        ...
        if (e.isVariableDefined(cpe)) {
            ...
            var enrollmentData = window.enrollmentsByCategory || {};
            var categories = Object.keys(enrollmentData);
            var enrollmentValues = categories.map(function(category) {
                return enrollmentData[category] || 0;
            });

            // Check if there is any data
            if (categories.length === 0 || enrollmentValues.every(value => value === 0)) {
                // Display "No Data" message
                cpe.innerHTML = '<div class="text-center p-4"><h5 class="text-muted">Không có học viên nào đăng ký trong thời gian này</h5></div>';
                return;
            }
            ...
            var options = {
                ...
                tooltip: {
                    y: {
                        formatter: function(val) {
                            return val.toLocaleString() + ' học viên';
                        }
                    }
                },
                colors: [
                    ...
                    '#00E396', '#FEB019', '#FF4560', '#775DD0', '#546E7A', '#26a69a'
                ]
            };

            window.enrollmentChart = new ApexCharts(cpe, options);
            window.enrollmentChart.render();
        }
    },
    // END: Earning Chart
    ...
    // START: Maintenance Revenue Chart
    maintenanceChart: function() {
        var maintenanceChart = e.select('#ChartMaintenanceRevenue');
        if (e.isVariableDefined(maintenanceChart)) {
            // Destroy existing chart if any
            if (window.maintenanceChart) {
                window.maintenanceChart.destroy();
            }

            var maintenanceData = window.maintenanceRevenue || {};
            var months = Object.keys(maintenanceData);
            ...
            months.forEach(function(month) {
                var data = maintenanceData[month];
                if (data) {
                    revenueValues.push(data.revenue || 0);
                    enrollmentValues.push(data.enrollmentCount || 0);
            ...
            var options = {
                ...
                yaxis: [{
                    title: {
                        text: 'Doanh Thu (VNĐ)',
                        style: {
                            color: ThemeColor.getCssVariableValue('--bs-primary')
                        }
                    },
                    labels: {
                        formatter: function(value) {
                    ...
                    title: {
                        text: 'Số Học Viên',
                        style: {
                            color: ThemeColor.getCssVariableValue('--bs-success')
                        }
                    },
                    labels: {
                        formatter: function(value) {
                ...
                tooltip: {
                    y: [{
                        ...
                        formatter: function(value) {
                            return value.toLocaleString('vi-VN') + ' học viên';
                        }
                    }]
                },
                colors: [
            ...
            window.maintenanceChart = new ApexCharts(document.querySelector("#ChartMaintenanceRevenue"), options);
            window.maintenanceChart.render();
        }
    },
    // END: Maintenance Revenue Chart
