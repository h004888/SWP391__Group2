...
function loadUsers(roleId, keyword = '') {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            ...
            console.log("Data:", {keyword: keyword || null, role: roleId});
        },
        success: function (data) {
            if (tableBodyElement) {
                ...
                if (rows.length === 0 || (rows.length === 1 && rows[0].textContent.includes('Không tìm thấy'))) {
                    console.warn("No data rows found!");
                }
                updateCountBadge(roleId, tableBodyElement.innerHTML);
            }
            console.log("Users loaded successfully for role:", roleId);
        },
